  ...
  for (int row = 0; row < BOARD_SIZE; row++) {
    for (int col = 0; col < BOARD_SIZE; col++) {
        ...
        if (gameBoard.change[row * 8 + col] == '+') {
          ...
          SDL_RenderFillCircle(renderer, 20 + col * CELL_SIZE + CELL_SIZE / 2, 20+ row * CELL_SIZE + CELL_SIZE / 2, CELL_SIZE / 2 - 1);
        }
        if (gameBoard.nowBoard[row * 8 + col] == 'X') {
          SDL_SetRenderDrawColor(renderer, COLOR_BLACK.r, COLOR_BLACK.g, COLOR_BLACK.b, COLOR_BLACK.a);
          SDL_RenderFillCircle(renderer, 20 + col * CELL_SIZE + CELL_SIZE / 2, 20+ row * CELL_SIZE + CELL_SIZE / 2, CELL_SIZE / 2 - 3);
        }
