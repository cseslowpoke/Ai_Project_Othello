...
public:
  ...
  min_max_monte_carlo(int _player, int _min_max_depth, int _monte_carlo_times)
      : ai(_player) {
    min_max_depth = _min_max_depth;
    monte_carlo_times = _monte_carlo_times;
    srand(time(NULL));
  }
  void name() { cout << "hua" << endl; }
  int move(std::string board_) {
    ...
    // "\n";
    // cerr << "time: " << chrono::duration_cast<chrono::microseconds>(end -
    // start).count() << "\n";
    // cerr<<visit<<"\n";
    return res.first;
  }
  ...
  double heuristic(bitboard gb) {
    // return __builtin_popcountll(gb.playerBoard) -
    // __builtin_popcountll(gb.opponentBoard);
  ...
  pair<int, double> minimax(bitboard gb, int depth, int mode, double l_lim,
                            double r_lim) {
    // visit++;
